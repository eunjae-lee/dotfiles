...
- gh pr view --json title,body,state,url,baseRefName,headRefName,author
...
2. If an `agents/` directory exists in the repository, invoke `/check-agents-rules` to check compliance against the team's rules.
3. Review the diff against these baseline rules:
